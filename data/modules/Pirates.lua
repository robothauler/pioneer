...
local calculateThread = function (lawlessness)
...
local onEnterSystem = function (player)
	...
	while max_pirates > 0 do
		...
		if Engine.rand:Number(1) < lawlessness then
			local threat = calculateThread(lawlessness)
...
local onShipDestroyed = function (ship)
	if pirates[ship] then
		...
		-- Spawn a new pirate
		local threat = calculateThread(Game.system.lawlessness)
		local planet = planets[Engine.rand:Integer(1, #planets)]:GetSystemBody()
		...
		newPirate:AIEnterMediumOrbit(planet.body)
	end
end
